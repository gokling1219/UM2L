...
parser.add_argument("-e","--episode",type = int, default= 40000)
#-----------------------------------------------------------------------------------#
...
def train():
    ...
    # 训练数据集
    f = h5py.File(r'./data/train_40000_47040.h5', 'r')
    ...
    for episode in range(EPISODE):
        ...
        support = np.uint8(support.reshape(n_way * n_shot, depth, im_height, im_width).transpose((0,2,3,1))*255)
        support_image = np.zeros([n_way*n_shot, depth, im_height, im_width])
        for i, each in enumerate(support):
            support_image[i] =  train_transform(Image.fromarray(each)).numpy()

        support_tensor = torch.from_numpy(support_image).type(torch.FloatTensor)
        #print("support_tensor.shape", support_tensor.shape) # (20, 3, 28, 28)

        query = np.uint8(query.reshape(n_way * n_query, depth, im_height, im_width).transpose((0,2,3,1))*255)
        query_image = np.zeros([n_way*n_query, depth, im_height, im_width])
        for i, each in enumerate(query):
            query_image[i] =  train_transform(Image.fromarray(each)).numpy()

        query_tensor = torch.from_numpy(query_image).type(torch.FloatTensor)
    ...
    torch.save(feature_encoder.state_dict(), str('./model/CROSS_feature_encoder_' +str(n_way) + 'way_' + str(n_shot) + 'shot_' + str(EPISODE)
                                                 + 'epoch_' +str(num_fea) + 'conv_lr' + str(LEARNING_RATE) + '.pkl'))
    torch.save(relation_network.state_dict(), str('./model/CROSS_relation_network_' +str(n_way) + 'way_' + str(n_shot) + 'shot_' + str(EPISODE)
                                                  + 'epoch_' + str(num_fea) + 'conv_lr' + str(LEARNING_RATE) + '.pkl'))


    f = open('./record/metalearning_loss_' +str(n_way) + 'way_' + str(n_shot) + 'shot_' + str(EPISODE)
                                                 + 'epoch_' +str(num_fea) + 'conv_lr' + str(LEARNING_RATE) + '.txt', 'w')
    for i in range(np.array(loss_).shape[0]):
        f.write(str(loss_[i]) + '\n')
    f = open('./record/metalearning_accuracy_' +str(n_way) + 'way_' + str(n_shot) + 'shot_' + str(EPISODE)
                                                 + 'epoch_' +str(num_fea) + 'conv_lr' + str(LEARNING_RATE) + '.txt', 'w')
    for i in range(np.array(accuracy_).shape[0]):
        f.write(str(accuracy_[i]) + '\n')

    f = open('./record/metalearning_time' + str(EPISODE)+ 'epoch' + '.txt', 'w')
